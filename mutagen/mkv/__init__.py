...
class MKVMetadataError(error):
    pass


class MKVStreamInfoError(error):
    pass


class MKVMetadataValueError(ValueError, MKVMetadataError):
    pass

__all__ = ['MKV', 'Open', 'delete', 'MKVCover', 'MKVFreeForm', 'AtomDataType','MediaKind', 'HDVideo', 'ContentRating']


@enum
class AtomDataType(object):
    """Enum for `dataformat` attribute of MKVFreeForm.
...
class MKVCover(bytes):
    ...
    * imageformat -- format of the image (either FORMAT_JPEG or FORMAT_PNG)
    ...
    def __eq__(self, other):
        if not isinstance(other, MKVCover):
...
class MKVFreeForm(bytes):
    ...
    * dataformat -- format of the data (see AtomDataType)
    ...
    def __eq__(self, other):
        if not isinstance(other, MKVFreeForm):
            return NotImplemented
...
class MKVTags(DictProxy, Metadata):
    ...
    * 'covr' -- cover artwork, list of MKVCover objects (which are
      tagged strs)
    * 'gnre' -- ID3v1 genre. Not supported, use '\\xa9gen' instead.
    ...
    MKV tag data cannot exist outside of the structure of an MKV file,
    ...
    def __init__(self, *args, **kwargs):
        self._failed_atoms = {}
        super(MKVTags, self).__init__(*args, **kwargs)

    def load(self, atoms, fileobj):
        try:
            ilst = atoms[b"moov.udta.meta.ilst"]
        except KeyError as key:
            raise MKVMetadataError(key)
        for atom in ilst.children:
            ok, data = atom.read(fileobj)
            if not ok:
                raise MKVMetadataError("Not enough data")
    ...
    def __setitem__(self, key, value):
        ...
        super(MKVTags, self).__setitem__(key, value)
    ...
    def __parse_freeform(self, atom, data):
        ...
        while pos < atom.length - 8:
            ...
            value.append(MKVFreeForm(data[pos + 16:pos + length],
    ...
    def __parse_genre(self, atom, data):
        ...
        for version, flags, data in self.__parse_data(atom, data):
            # version = 0, flags = 0
            if len(data) != 2:
                raise MKVMetadataValueError("invalid genre")
            ...
            try:
                ...
                raise MKVMetadataValueError("unknown genre")
            values.append(genre)
    ...
    def __parse_tempo(self, atom, data):
        ...
        for version, flags, data in self.__parse_data(atom, data):
            ...
            if len(data) != 2:
                raise MKVMetadataValueError("invalid tempo")
    ...
    def __parse_bool(self, atom, data):
        for version, flags, data in self.__parse_data(atom, data):
            if len(data) != 1:
                raise MKVMetadataValueError("invalid bool")
    ...
    def __parse_cover(self, atom, data):
        ...
        while pos < atom.length - 8:
            ...
            if imageformat not in (MKVCover.FORMAT_JPEG, MKVCover.FORMAT_PNG):
                # Sometimes AtomDataType.IMPLICIT or simply wrong.
                # In all cases it was jpeg, so default to it
                imageformat = MKVCover.FORMAT_JPEG
            cover = MKVCover(data[pos + 16:pos + length], imageformat)
            values.append(cover)
            pos += length
    ...
    def __parse_text(self, atom, data, implicit=True):
        ...
        for version, flags, atom_data in self.__parse_data(atom, data):
            ...
            try:
                ...
                raise MKVMetadataError("%s: %s" % (_name2key(atom.name), e))
...
class MKVInfo(StreamInfo):
    ...
    * codec_description (string):
    ...
    def __init__(self, atoms, fileobj):
        ...
        #     moov = atoms[b"moov"]
        # except KeyError:
        #     raise MKVStreamInfoError("not a MKV file")
        #
        # for trak in moov.findall(b"trak"):
        #     hdlr = trak[b"mdia", b"hdlr"]
        #     ok, data = hdlr.read(fileobj)
        #     if not ok:
        #         raise MKVStreamInfoError("Not enough data")
        #     if data[8:12] == b"soun":
        #         break
        # else:
        #     raise MKVStreamInfoError("track has no audio data")
        ...
        #     fmt = ">IQ"
        # else:
        #     raise MKVStreamInfoError("Unknown mdhd version %d" % version)
        #
    ...
    def _parse_stsd(self, atom, fileobj):
        ...
        ok, data = atom.read(fileobj)
        if not ok:
            raise MKVStreamInfoError("Invalid stsd")
        ...
        if version != 0:
            raise MKVStreamInfoError("Unsupported stsd version")
...
class MKV(FileType):
    ...
    :ivar info: :class:`MKVInfo`
    :ivar tags: :class:`MKVTags`
    """
    ...
    def load(self, filename):
        ...
        with open(filename, "rb") as fileobj:
            ...
            try:
                self.info = MKVInfo(atoms, fileobj)
            except error:
                raise
            except Exception as err:
                reraise(MKVStreamInfoError, err, sys.exc_info()[2])

            if not MKVTags._can_load(atoms):
                self.tags = None
            else:
                try:
                    self.tags = self.MKVTags(atoms, fileobj)
                except error:
                    raise
                except Exception as err:
                    reraise(MKVMetadataError, err, sys.exc_info()[2])

    def add_tags(self):
        if self.tags is None:
            self.tags = self.MKVTags()
        else:
            raise error("an MKV tag already exists")
