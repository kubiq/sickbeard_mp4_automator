...
class tmdb_mkv:
    ...
    def getArtwork(self, mkvPath, filename='cover'):
        # Check for local artwork in the same directory as the mkv
...
def main():
    if len(sys.argv) > 2:
        ...
        tmdb_mkv_instance = tmdb_mkv(imdb_id)
        if os.path.splitext(mkv)[1][1:] in valid_output_extensions:
            tmdb_mkv_instance.writeTags(mkv)
        else:
