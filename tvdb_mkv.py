...
from mutagen.mkv import MKV, MKVCover
...
class Tvdb_mkv:
    ...
    def writeTags(self, mkvPath, artwork=True, thumbnail=False):
        self.log.info("Tagging file: %s." % mkvPath)
        ext = os.path.splitext(mkvPath)[1][1:]
        ...
        if artwork:
            path = self.getArtwork(mkvPath, thumbnail=thumbnail)
            if path is not None:
                cover = open(path, 'rb').read()
                if path.endswith('png'):
                    video["covr"] = [MKVCover(cover, MKVCover.FORMAT_PNG)]  # png poster
                else:
                    video["covr"] = [MKVCover(cover, MKVCover.FORMAT_JPEG)]  # jpeg poster
        if self.original:
            video["\xa9too"] = "MDH:" + os.path.basename(self.original)
        else:
            video["\xa9too"] = "MDH:" + os.path.basename(mkvPath)
        MKV(mkvPath).delete(mkvPath)
        for i in range(3):
            try:
...
def main():
    if len(sys.argv) > 4:
        mkv = str(sys.argv[1]).replace("\\", "\\\\").replace("\\\\\\\\", "\\\\")
        tvdb_id = str(sys.argv[2])
        if tvdb_id.isdigit():
            tvdb_id = int(tvdb_id)
        tvdb_season = int(sys.argv[3])
        tvdb_episode = int(sys.argv[4])
        tvdb_mkv_instance = Tvdb_mkv(show=tvdb_id, season=tvdb_season, episode=tvdb_episode)
        if os.path.splitext(mkv)[1][1:] in valid_output_extensions:
            tvdb_mkv_instance.writeTags(mkv)
        else:
